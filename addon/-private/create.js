...
// it (the "chained tree") and set it as the '_chainedTree' property on the root
// of the main tree. This allows us to find a node's root and look for that
// property to determine if it's in the chained tree or not. Anytime a chainable
// method is invoked, instead of returning the node whose method was invoked, we
// can return its mirror node in the chained tree. Then, anytime an async method
// is invoked on that node (meaning we are in a chaining scenario), the
// execution context can recognize it as a chained node and wait before invoking
// the target method.
//
